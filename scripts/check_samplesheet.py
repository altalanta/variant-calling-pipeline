...
def check_samplesheet(file_in, file_out):
    ...
    with open(file_in, "r") as fin:
        ...
        unknown_columns = [reader.fieldnames[i] for i, col in enumerate(normalized_header) if col not in valid_normalized]
        if unknown_columns:
            print(f"Warning: Unknown column(s) found: {', '.join(unknown_columns)}", file=sys.stderr)
        ...
        for line_num, row in enumerate(reader, 2):  # Start at line 2 (after header)
            ...
            if read1 and not any(read1.endswith(ext) for ext in valid_extensions):
                print_error(f"read1 file has invalid extension: {read1}. Expected: {', '.join(valid_extensions)}", "Line", line_num)
            if read2 and not any(read2.endswith(ext) for ext in valid_extensions):
                print_error(f"read2 file has invalid extension: {read2}. Expected: {', '.join(valid_extensions)}", "Line", line_num)
            ...
            if platform and platform.upper() not in valid_platforms:
                print(f"Warning: Unknown platform '{platform}' on line {line_num}. Valid platforms: {', '.join(valid_platforms)}", file=sys.stderr)
            ...
            if not lane:
                ...
                print(f"Warning: Lane '{lane}' is not numeric on line {line_num}. Setting to '1'.", file=sys.stderr)
                lane = "1"

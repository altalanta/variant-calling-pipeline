...
class VCFQualityControl:
    ...
    def generate_plots(self):
        ...
        if self.variant_types:
            axes[0,0].pie(self.variant_types.values(), labels=self.variant_types.keys(), autopct='%1.1f%%')
    ...
    def generate_report(self):
        ...
        with open(report_file, 'w') as f:
            f.write(f"VCF Quality Control Report\n")
            f.write(f"=" * 50 + "\n")
            ...
            f.write(f"SNPs: {self.stats['snps']:,} ({self.stats['snps']/max(1,self.stats['total_variants'])*100:.1f}%)\n")
            f.write(f"Indels: {self.stats['indels']:,} ({self.stats['indels']/max(1,self.stats['total_variants'])*100:.1f}%)\n")
            f.write(f"Multiallelic sites: {self.stats['multiallelic']:,}\n")
            f.write(f"Passed filters: {self.stats['passed']:,} ({self.stats['passed']/max(1,self.stats['total_variants'])*100:.1f}%)\n")
            f.write(f"Filtered out: {self.stats['filtered']:,} ({self.stats['filtered']/max(1,self.stats['total_variants'])*100:.1f}%)\n")
...
"""
    ...
    parser.add_argument(
        '--plots',
        action='store_true',
        help='Generate quality control plots'
    )
    
    parser.add_argument(
        '--json',
        action='store_true',
        help='Save statistics as JSON file'
    )
    ...
    parser.add_argument(
        '--expected-variants',
        type=int,
        help='Expected number of variants (for testing)'
    )

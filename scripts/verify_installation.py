...
def run_basic_syntax_check():
    ...
    try:
        ...
        ], capture_output=True, text=True, timeout=30, cwd=workflow_dir)
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify pipeline installation and dependencies"
    )
    parser.add_argument(
        '--check-docker-pull', 
        action='store_true',
        help='Also check if Docker image can be pulled'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true', 
    ...
    if all_good:
        ...
        print("  nextflow run workflow/main.nf -profile docker --input tests/smoke/samplesheet.csv --outdir results --ref_fasta tests/smoke/ref/test_ref.fa")
    else:
